...
Table: Fuentes de avisos laborales utilizadas. En _Nombre_ se define el tipo de información que contiene la fuente de datos. En _Fuente_ se define la fuente de datos utilizada, es decir, donde se generó la información. En _Extraida_ se menciona donde fueron recabados los datos física o virtualmente. _Periodo_ refiere a los años a los cuales pertenecen las fuentes de datos. \label{fuentes}
...
```{r ga13-18-comparacion, fig.align="center", fig.cap="Series de avisos laborales de Gallito entre 2013 y 2018, los datos fueron compartidos por parte de el diario El País, procesamiento propio. Las series refieren a los avisos publicados filtrados por link (id) de aviso y a la cantidad de publicaciones sin filtrar. Se observa una diferencia de nivel relativamente estable."}
...
```{r iecon-ceres, fig.cap="Tasas de crecimiento de series de vacantes laborales anualizadas obtenidas a partir de IECON y CERES en los años 2000 a 2009, procesamiento propio. Se observa una elevada correlación lineal entre las tasas de crecimiento en el orden del 90\\%.", fig.align="center"}
...
```{r series-conjuntas, fig.cap = "Series individuales de avisos laborales sin corregir. La serie de color negro corresponden Urrestarazu (1980.1995) y elaboración propia (1995-1998, 1999Q1, 2000) más una predicción de dos años. Naranja al ICDL, color verde a Gallito facilitado por diario El País y con color rojo y azul a Buscojobs y Computrabajo respectivamente los cuales son de construcción propia, la linea y puntos grises corresponden a avisos trimestrales de Gallito de recolección propia. Todas las series son de construcción propia."}
col_um           = "black"
...
```{r serie-final-gallito, fig.cap = "Serie trimestral de avisos laborales publicados en el portal Gallito. Años 1980-1995 datos extraídos de Urrestarazu, entre 1995 y 1998 recolección propia, 1998-2012 se utilizan los datos del ICDL, entre 2013 y 2018 datos provistos por diario El País, año 2019 obtenido mediante scraping web, construcción propia. Con coloz azul la serie final de Gallito, con color naranja serie de Gallito entre 2013 y 2018, color rojo serie avisos obtenidos mediante el ICDL y con color verde la serie combinada de Urrestarazu y Molina (construcción propia)."}
...
```{r serie-buscojobs, fig.cap="Serie trimestral de avisos laborales publicados en el portal Buscojobs, construcción y elaboración propia. Los datos fueron obtenidos a través del portal waybackmachine, scraping de Buscojobs y finalmente imputando los valores faltantes."}
...
```{r serie-computrabajo, fig.cap="Serie trimestral de avisos laborales publicados en el portal Computrabajo, construcción y elaboración propia. Los datos fueron obtenidos a través del portal waybackmachine, scraping de Computrabajo y finalmente imputando los valores faltantes."}
bj_ct[, 
...
```{r molina96-98, fig.cap="Publicaciones Gallito, series de avisos y puestos laborales de frecuencia mensual. Recolección y construcción propia. Se observa una diferencia de nivel entre las series de puestos y avisos laborales, ambas siguen el mismo movimiento. La serie de puestos se construyo contabilizando todos los puestos solicitados en la sección de avisos destacados, en el resto de las secciones todos los avisos se contabilizaron con un puesto.", fig.align="center"}
...
```{r iecon-avisos-puestos, fig.cap="Serie de puestos y avisos laborales con frecuencia anual. Construida en base a los datos IECON para el periodo 2000-2009, construcción propia. Se observa un comovomiento entre avisos y puestos, donde la diferencia es de nivel.", fig.align="center"}
...
4. Existe un sesgo hacia puestos laborales de baja calificación. Se observa para el periodo 2013-2018 en base al análisis \@ref(fig:ga-13-18-areas), que la mayor cantidad de avisos corresponden a puestos de auxiliar, técnico/especialista, ejecutivo comercial y peón. Si bien, esto no es una muestra del sesgo, puesto que los puestos de mayor jerarquía son por definición menor a los puestos de menor calificación, por lo cual una simple comparación porcentual no nos dice nada. @Alma2011 utilizando la información de puestos laborales para el periodo 2000-2009, utilizando la misma fuente de datos realizan un análisis pormenorizado de la información publicada en cada aviso laboral y lo comparan con la ECH, concluyendo que _Gallito_ tiene sesgo hacia puestos de baja calificación. Es posible extender el análisis de @Alma2011 y replicarlo para el periodo 2013-2018.
